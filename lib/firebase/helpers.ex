defmodule Firebase.Helper do
  ...
  def custom_token_to_id_or_refresh_token(token) when is_binary(token) do
    {:ok, output} = HTTPoison.post(url_custom_to_id_token(), encode_custom_token_to_id_or_refresh_token(token), @headers)

    response = 
      output
      |> Map.from_struct()
    
    changed = Poison.decode!(response.body) 

    Map.replace!(response, :body, changed)
  end
